    ...
    return (
        <div className="bg-gray-200 min-h-screen">
            ...
            <div className='flex flex-wrap justify-center text-center bg-gray-200' style={{ direction: 'rtl' }}>
                {recepies.map((recipe) => (
                    <div key={recipe.Id} className="bg-yellow-100  lg:w-1/4 md:w-1/3 sm:w-1/2 p-4 m-2 shadow-md text-gray-800 rounded-lg hover:shadow-2xl transition-shadow duration-300 flex flex-col justify-between w-80">
                        <div>
                            <p className="break-words whitespace-pre-wrap">המתכון הועלה בתאריך {new Date(recipe.Date).toLocaleDateString()}</p>
                            <p className="break-words whitespace-pre-wrap">ע"י {recipe.IdUserNavigation.Name}</p>
                            <h2 className='font-bold text-xl border-b border-black text-gray-800 mb-1 break-words whitespace-pre-wrap'>{recipe.Name}</h2>
